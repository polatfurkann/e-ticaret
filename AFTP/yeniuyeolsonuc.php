...
if(($GelenEmailAdresi!="") and ($GelenSifre!="") and ($GelenSifreTekrar!="") and ($GelenIsimSoyisim!="") and ($GelenTelefonNumarasi!="") and ($GelenCinsiyet!="")){
    if($GelenSozlesmeOnay==0){
        ...
        if($GelenSifre!=$GelenSifreTekrar){
            ...
            $KontrolSorgusu->execute([$GelenEmailAdresi]);  //Çalıştır
            $KullaniciSayisi    = $KontrolSorgusu->rowCount(); //Kayıt var mı yok mu?
            ...
            if($KullaniciSayisi>0){
                ...
                $UyeEklemeSorgusu->execute([$GelenEmailAdresi, $MDB5liSifre, $GelenIsimSoyisim, $GelenTelefonNumarasi, $GelenCinsiyet, 0, $ZamanDamgasi, $IPAdresi, $AktivasyonKodu ]);  
